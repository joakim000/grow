...
impl grow::ops::running::TextDisplay for Oled {
    fn init(
        ...
        let mut display: Ssd1306<I2CInterface<I2c>, DisplaySize128x64, ssd1306::mode::BufferedGraphicsMode<DisplaySize128x64>> = 
...
impl  Oled {
    ...
    fn write_test(
        mut display: Ssd1306<I2CInterface<I2c>, DisplaySize128x64, ssd1306::mode::BufferedGraphicsMode<DisplaySize128x64>>,
        ...
        let next = Text::new("small ", Point::new(20, 30), character_style_small).draw(&mut display).unwrap();
        // Draw the second text after the first text using the large character style.
        let next = Text::new("large", next, character_style_large).draw(&mut display).unwrap();
        display.flush().unwrap();
