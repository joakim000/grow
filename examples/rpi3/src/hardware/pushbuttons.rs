use core::error::Error;
use tokio::task::JoinHandle;
use tokio_util::sync::CancellationToken;
use async_trait::async_trait;
use tokio::sync::broadcast::error::SendError;
use rppal::gpio::{Gpio, Trigger, Level};
use tokio::time::sleep;
use core::time::Duration;

use super::conf::{BUTTON_1_PIN, BUTTON_2_PIN};
use grow::ops::io::{ButtonPanel, ButtonInput};

pub struct PushButtons {
    // id: u8,
    // hub: HubMutex,
    feedback_task: Option<JoinHandle<()>>,
    cancel: CancellationToken,
}
// #[async_trait]
impl ButtonPanel for PushButtons {
    // fn id(&self) -> u8;
    fn init(
        &mut self,
        from_buttons: tokio::sync::broadcast::Sender<ButtonInput>,
    ) -> Result<(), Box<dyn Error>> {

        let cancel = self.cancel.clone();
        let button_handler = tokio::spawn(async move {
            let mut btn_1 = Gpio::new().expect("GPIO error").get(BUTTON_1_PIN).expect("GPIO pin error").into_input_pulldown();
            let mut btn_2 = Gpio::new().expect("GPIO error").get(BUTTON_2_PIN).expect("GPIO pin error").into_input_pulldown();
            let from_btn1 = from_buttons.clone();
            let from_btn2 = from_buttons.clone();
            btn_1.set_async_interrupt(Trigger::Both, move |l| { 
                // println!("Btn 111: {:?}", &l);
                match l {
                    Level::High => {from_btn1.send(ButtonInput::OneDown); }
                    Level::Low => {from_btn1.send(ButtonInput::OneUp); }
                }
            });
            btn_2.set_async_interrupt(Trigger::Both, move |l| { 
                // println!("Btn 2 2 2: {:?}", &l);
                match l {
                    Level::High => {from_btn2.send(ButtonInput::TwoDown); }
                    Level::Low => {from_btn2.send(ButtonInput::TwoUp); }
                }
            });

            // println!("Button pins initialized");
            cancel.cancelled().await;
        });
        
        Ok(())
    }
}

impl PushButtons {
    pub fn new(cancel: CancellationToken) -> Self {
        Self {
            // id,
            feedback_task: None,
            cancel,
        }
    }
}
