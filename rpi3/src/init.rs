...
// pub async fn hardware_init(pu: Arc<TokioMutex<PoweredUp>>) -> (HouseMutex, ManagerMutex) {
pub async fn hardware_init(cancel: CancellationToken) -> (HouseMutex, ManagerMutex) {
        ...
        match zone {
            ...
            Zone::Aux {
                ...
                interface.aux_device =
                    Some(Box::new(hardware::lpu::LpuHub::new(*id, lpu_hub.clone(), cancel.clone())));
            }
            Zone::Light {
                id,
                settings: _,
    ...
    let (zones_tx, zones_rx) = grow::zone::zone_channels();
    ...
    
    {
        ...
        lock.init(zones_rx, ops_tx.clone(), ops_rx, manager_mutex.clone());
        // dbg!(&lock);
    }
    {
        let mut lock = house_mutex.lock().await;
        lock.init(zones_tx, ops_tx).await;
        // dbg!(&lock);
    }
